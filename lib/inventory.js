...
export async function searchInventoryForAI({ category, maxPrice, minPrice, tags }) {
  ...
  return data.map(p => ({
    ...
    specs: p.product_attributes?.slice(0, 4).map(a =>
...
export async function generateAIBaseline() {
  const { data, error } = await supabase
    .from('products')
    .select('id, name, price, is_active, categories(name), product_attributes(value, attribute_definitions(name))')
    .eq('is_active', true);
  ...
  let productsList = "";
  if (data.length <= 150) {
    productsList = "\n\n### CATÁLOGO DETALLADO (RECOMENDAR ESTRICTAMENTE SOLO ESTOS):\n" +
      data.map(p => {
        ...
        const gpu = specs.find(s => s.attribute_definitions.name.toLowerCase().includes('gpu'))?.value || 'n/a';
        const storage = specs.find(s => s.attribute_definitions.name.toLowerCase().includes('almacenamiento'))?.value || 'n/a';
        const screen = specs.find(s => s.attribute_definitions.name.toLowerCase().includes('pantalla'))?.value || 'n/a';
        const priceLabel = p.price !== null && p.price !== undefined && !isNaN(parseFloat(p.price)) ? parseFloat(p.price).toLocaleString() : '??';
        const modelLabel = p.model_number ? ` [Modelo: ${p.model_number}]` : '';

        return `- ${p.name}${modelLabel} | $${priceLabel} | CPU: ${cpu} | RAM: ${ram}GB | SSD: ${storage}GB | Pantalla: ${screen}" | [ID: ${p.id}]`;
      }).join('\n');
  }

  return `### BANANA COMPUTER - CEREBRO OPERATIVO
Resumen de Stock Actual:
- Total productos activos: ${data.length}
- Categorías: ${categories.join(', ')}
- Rango Precios: $${minPrice.toLocaleString()} a $${maxPrice.toLocaleString()}

### REGLAS DE NEGOCIO:
- Si el cliente busca algo especializado (Gaming, Diseño, Oficina), prioriza los modelos que tengan las specs adecuadas.${productsList}

### FIN DEL CONOCIMIENTO BASE.`;
}
