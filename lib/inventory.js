...
export async function generateAIBaseline() {
  ...
  let productsList = "";
  if (data.length <= 150) { // Increased limit slightly for Next.js scale
    productsList = "\nPRODUCTOS ACTUALES EN STOCK (con IDs para RECOMENDACION):\n" + 
      data.map(p => {
  ...
  return `Banana Computer Baseline Information:
- Total productos activos: ${data.length}
- Categorías disponibles: ${categories.join(', ')}
- Rango de precios: $${minPrice.toLocaleString()} a $${maxPrice.toLocaleString()}${productsList}
- Nota: Actualmente NO vendemos computadoras de escritorio (PC Desktop). Solo Laptops.
- Si ves una opción que calza perfecto, recomiéndala de inmediato y usa su ID oficial.`;
}
