...
export async function searchInventoryForAI({ category, maxPrice, minPrice, tags }) {
  let query = supabase
    .from('products')
    .select('id, slug, name, price, description, tagline, is_active, categories(name, slug), subcategories:subcategory_id(name, slug), product_attributes(value, attribute_definitions(name, unit))')
    .eq('is_active', true);
  ...
  return filtered.slice(0, 5).map(p => ({
    id: p.id,
    slug: p.slug,
    category_slug: p.categories?.slug || 'c',
    subcategory_slug: p.subcategories?.slug || 's',
    name: p.name,
    price: p.price,
...
export function formatInventoryForAI(products) {
  ...
  return products.map(p =>
    `- SLUG: ${p.slug} | ${p.name} | $${p.price} | Specs: ${p.specs}`
  ).join('\n');
}
...
export async function generateAIBaseline() {
  const { data, error } = await supabase
    .from('products')
    .select('id, slug, name, price, model_number, is_active, categories(name, slug), subcategories:subcategory_id(name, slug), product_attributes(value, attribute_definitions(name))')
    .eq('is_active', true);
  ...
  let productsList = "";
  if (data.length <= 150) {
    productsList = "\n\n### CATÁLOGO (SLUG | NOMBRE | PRECIO | SPECS):\n" +
      data.map(p => {
        ...
        return `- [SLUG: ${p.slug}] | ${p.name} | [Modelo: ${p.model_number || 'n/a'}] | $${priceLabel} | CPU: ${cpu}, RAM: ${ram}GB, SSD: ${storage}GB`;
      }).join('\n');
  }
