    ...
    <div className="min-h-screen bg-cream-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-[2.5rem] p-10 shadow-3xl border border-black/5 relative z-10 animate-in fade-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-3xl font-black tracking-tight">{isSignup ? 'Únete a la elite' : 'Bienvenido de nuevo'}</h1>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-2">{isSignup ? 'Crea tu cuenta oficial' : <>Ingresa a <span className="font-pixel text-purple-brand/60">Banana Computer</span></>}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <button 
            type="submit" 
            className="w-full py-4 bg-purple-brand text-white rounded-2xl font-black text-lg hover:scale-102 active:scale-95 transition-all shadow-xl shadow-purple-brand/20 disabled:opacity-50"
            disabled={loading}
          >
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-3 py-4 border border-black/5 rounded-2xl font-black text-sm hover:bg-gray-50 transition-all"
        >
        ...
        {signupSuccess && (
          <div className="mt-8 p-6 bg-mint-success/5 border border-mint-success/10 rounded-2xl flex flex-col items-center text-center gap-2 animate-in fade-in slide-in-from-bottom-2">
            ...
            <p className="text-xs font-bold text-mint-success leading-relaxed">
        ...
        <div className="mt-10 flex flex-col items-center gap-2">
          {!isSignup && (
            <Link href="/forgot-password" size="sm" className="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-purple-brand transition-colors">
              ¿Olvidaste tu contraseña?
            </Link>
          )}
          <button
            className="text-xs font-black text-purple-brand hover:underline"
        ...
        <Link href="/" className="mt-10 flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-300 hover:text-black transition-colors">
